...
Each field in a Contact is implemented as a separate class (e.g. a `Name` object represents the person's name).
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Contact editedContact = new Contact(personToEdit.getName(), personToEdit.getPhone(), personToEdit.getEmail(),
